...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("lightpay"))
        return false;
...
{
    // Convert lightpay:// to lightpay:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("lightpay://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "lightpay:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Lightpay.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Lightpay.lnk
...
{
    return GetAutostartDir() / "lightpay.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a lightpay.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Lightpay\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the lightpay app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add lightpay app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Lightpay-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  lightpay-qt [" + tr("command-line options") + "]                     " + "\n";
