...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeLightpayAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLightpayAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeLightpayAmount = strAnonymizeLightpayAmount.remove(strAnonymizeLightpayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLightpayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeLightpayAmount*COIN) nMaxToAnonymize = nAnonymizeLightpayAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeLightpayAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeLightpayAmount));
        strAnonymizeLightpayAmount = strAnonymizeLightpayAmount.remove(strAnonymizeLightpayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLightpayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
