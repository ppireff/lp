...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: lightpay.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: lightpayd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizelightpayamount=<n> " + _("Keep N Lightpay anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Lightpay is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Lightpay is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Lightpay version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Lightpay server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Lightpay") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Lightpay to complete") << "\n";
    ...
    nAnonymizeLightpayAmount = GetArg("-anonymizelightpayamount", 0);
    if(nAnonymizeLightpayAmount > 999999) nAnonymizeLightpayAmount = 999999;
    if(nAnonymizeLightpayAmount < 2) nAnonymizeLightpayAmount = 2;
    ...
    LogPrintf("Anonymize Lightpay Amount %d\n", nAnonymizeLightpayAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1XLP+1000 == (.1XLP+100)*10
       10XLP+10000 == (1XLP+1000)*10
    */
