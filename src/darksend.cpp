...
std::string CDarksendPool::GetStatus()
{
    ...
    switch(state) {
        ...
        case POOL_STATUS_FINAXLPE_TRANSACTION:
...
void CDarksendPool::Check()
{
    ...
    if(fMasterNode) {
        ...
        if(state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions())
        {
            ...
            UpdateState(POOL_STATUS_FINAXLPE_TRANSACTION);
        }
    }

    // create the finalized transaction for distribution to the clients
    if(state == POOL_STATUS_FINAXLPE_TRANSACTION) {
        LogPrint("darksend", "CDarksendPool::Check() -- FINAXLPE TRANSACTIONS\n");
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Lightpay and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002XLP per transaction on average.
            */
            if(r <= 10)
            {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeLightpayAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
bool CDarksendPool::MakeCollateralAmounts()
{
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, nChangePos, strFail, coinControl, ONLY_NONDENOMINATED_NOT3333IFMN);
    if(!success){
        // if we failed (most likeky not enough funds), try to use denominated instead -
        // MN-like funds should not be touched in any case and we can't mix denominated without collaterals anyway
        LogPrintf("MakeCollateralAmounts: ONLY_NONDENOMINATED_NOT3333IFMN Error - %s\n", strFail);
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                nFeeRet, nChangePos, strFail, coinControl, ONLY_NOT3333IFMN);
        if(!success){
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100XLP+1 ( bit on if present )
    // bit 1 - 10XLP+1
    // bit 2 - 1XLP+1
    // bit 3 - .1XLP+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("lightpay-darksend");
